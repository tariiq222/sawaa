...
describe('ListBookingsHandler', () => {
  it('returns paginated bookings', async () => {
    ...
    const result = await new ListBookingsHandler(prisma as never, buildTenant() as never).execute({
      page: 1, limit: 10,
    });
  ...
  it('filters by status when provided', async () => {
    const prisma = buildPrisma();
    const handler = new ListBookingsHandler(prisma as never, buildTenant() as never);
