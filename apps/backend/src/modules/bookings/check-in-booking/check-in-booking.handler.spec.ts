...
describe('CheckInBookingHandler', () => {
  it('sets checkedInAt on CONFIRMED booking', async () => {
    ...
    await new CheckInBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'book-1', changedBy: 'user-42' });
  ...
  it('throws BadRequestException when booking is not CONFIRMED', async () => {
    ...
    await expect(
      new CheckInBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'book-1', changedBy: 'user-42' }),
  ...
  it('throws NotFoundException when booking not found', async () => {
    ...
    await expect(
      new CheckInBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'bad', changedBy: 'user-42' }),
...
describe('CheckInBookingHandler — status log', () => {
  it('writes a BookingStatusLog entry on check-in', async () => {
    ...
    const handler = new CheckInBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never);
