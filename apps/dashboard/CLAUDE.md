...
- **No `useTerminology` — removed as dead scaffolding (2026-06-22).** The old hook called `GET /public/verticals/:slug/terminology`, an endpoint the single-tenant backend never implemented; it sat inert and orphaned and was deleted. Sawa is single-tenant — use plain `t()` from `useLocale()` for all labels. Do NOT reintroduce a terminology hook.
