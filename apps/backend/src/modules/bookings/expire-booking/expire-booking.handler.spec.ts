...
describe('ExpireBookingHandler', () => {
  it('expires PENDING booking', async () => {
    ...
    await new ExpireBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'book-1', changedBy: 'user-42' });
  ...
  it('throws BadRequestException when booking is not PENDING', async () => {
    ...
    await expect(
      new ExpireBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'book-1', changedBy: 'user-42' }),
  ...
  it('throws NotFoundException when not found', async () => {
    ...
    await expect(
      new ExpireBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'bad', changedBy: 'user-42' }),
...
describe('ExpireBookingHandler — group booking statuses', () => {
  it('expires PENDING_GROUP_FILL booking', async () => {
    ...
    await new ExpireBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'book-1', changedBy: 'system' });
    expect(prisma.booking.update).toHaveBeenCalledWith(
      expect.objectContaining({ data: expect.objectContaining({ status: BookingStatus.EXPIRED }) }),
    );
  });
...
describe('ExpireBookingHandler — status log', () => {
  it('writes a BookingStatusLog entry on expire', async () => {
    const prisma = buildPrisma();
    const handler = new ExpireBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never);
