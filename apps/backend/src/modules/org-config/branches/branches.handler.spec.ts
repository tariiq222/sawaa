...
describe('CreateBranchHandler', () => {
  it('creates branch scoped by org when name is unique', async () => {
    ...
    const handler = new CreateBranchHandler(prisma as never, buildEventBus() as never);
  ...
  it('allows same branch name in two different orgs', async () => {
    ...
    const handlerA = new CreateBranchHandler(prismaA as never, buildEventBus() as never);
    const handlerB = new CreateBranchHandler(prismaB as never, buildEventBus() as never);
...
describe('UpdateBranchHandler', () => {
  it('updates branch when found in org', async () => {
    ...
    const handler = new UpdateBranchHandler(prisma as never, eventBus as never);
  ...
  it('throws NotFoundException when branch not found', async () => {
    const prisma = buildPrisma();
    prisma.branch.findFirst = jest.fn().mockResolvedValue(null);
    const handler = new UpdateBranchHandler(prisma as never, buildEventBus() as never);
