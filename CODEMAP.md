...
| `apps/mobile/` | App | Expo SDK 55, RN 0.83, Expo Router | — | Native mobile UI for clients | Not included in root pnpm workspace (run via `pnpm --filter=mobile`) |
...
| `packages/ui/` | Package | shadcn/ui, React | — | Shared UI primitives for dashboard + website | Mobile UI (mobile uses its own component library) |
| `packages/test-helpers-pw/` | Package | Playwright helpers | — | Shared e2e utilities: auth setup, tenant helpers, webhook idempotency, config | App-specific assertions, secrets, browser credentials |
...
| Change a dashboard page | `apps/dashboard/src/app/` or `apps/dashboard/src/components/` |
| Change a website page | `apps/website/src/app/` or `apps/website/src/components/` |
| Change a shared UI primitive | `packages/ui/src/components/` |
...
| Surface | Command |
|---|---|
| Backend handler/service/DTO | `pnpm --filter=backend test -- path/to/file.spec.ts` |
