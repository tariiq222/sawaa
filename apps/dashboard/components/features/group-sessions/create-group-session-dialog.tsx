        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          <FormSection title={t("groupSessions.section.basics")}>
            <div className="flex flex-col gap-4">
              <FormField
                label={t("groupSessions.form.employee")}
              ...
              <FormField
                label={t("groupSessions.form.service")}
              ...
              <FormField
                label={t("groupSessions.form.title")}
          ...
          <FormSection title={t("groupSessions.section.schedule")}>
            <div className="grid grid-cols-[1fr_auto] gap-4">
              <FormField
                label={t("groupSessions.form.scheduledAt")}
              ...
              <FormField
                label={t("groupSessions.form.durationMins")}
                error={errors.durationMins?.message as string}
              >
                <Input
                  type="number"
                  min={1}
          ...
          <FormSection title={t("groupSessions.section.capacityPrice")}>
            <div className="grid grid-cols-3 gap-4">
              <FormField
                label={t("groupSessions.form.maxCapacity")}
                error={errors.maxCapacity?.message as string}
              >
                <Input type="number" min={1} className="tabular-nums" {...register("maxCapacity")} />
              </FormField>
              <FormField
                label={t("groupSessions.form.price")}
                error={errors.priceInSar?.message as string}
              >
                <Input
                  type="number"
                  min={0}
          ...
          <FormSection title={t("groupSessions.section.details")}>
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <FormField label={t("groupSessions.form.descriptionAr")}>
                  <Textarea rows={2} {...register("descriptionAr")} />
                </FormField>
                <FormField label={t("groupSessions.form.descriptionEn")}>
              ...
              <div className="flex items-center justify-between gap-4 rounded-lg border border-border bg-muted/40 px-4 py-3">
                <div className="flex flex-col gap-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("groupSessions.form.isPublicHint")}
                  </p>
                </div>
                <Controller
                  control={control}
              ...
              >
                <div className="overflow-hidden">
                  <div className="grid grid-cols-2 gap-4 pt-1">
                    <FormField label={t("groupSessions.form.publicDescriptionAr")}>
                      <Textarea rows={2} {...register("publicDescriptionAr")} />
                    </FormField>
                    <FormField label={t("groupSessions.form.publicDescriptionEn")}>
          ...
          <DialogFooter className="flex-col items-stretch gap-2 sm:flex-row sm:items-center">
            ...
            <Button
              type="submit"
              size="sm"
              disabled={createMut.isPending || !branchReady}
            >
              {createMut.isPending
                ? t("groupSessions.create.submitting")
                : t("groupSessions.create.submit")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
