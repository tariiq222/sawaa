...
- Provider transcript trimming is group-aware: an assistant tool-call message and every matching `tool_call_id` result are retained together or omitted together. A malformed or individually oversized group is rejected before provider use. The maximum-size eight-call test asserts every retained tool result has a matching assistant call while the transcript remains within the same reservation-bound envelope.
