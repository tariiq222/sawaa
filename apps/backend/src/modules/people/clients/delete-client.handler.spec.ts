...
describe('DeleteClientHandler', () => {
  ...
  it('throws when client not found', async () => {
    ...
    await expect(handler.execute({ clientId: 'c1' })).rejects.toThrow(NotFoundException);
  });

  it('blocks when active bookings exist', async () => {
    prisma.client.findFirst.mockResolvedValue({ id: 'c1', phone: '+966500000000', notes: '' });
    prisma.booking.count.mockResolvedValue(2);
    await expect(handler.execute({ clientId: 'c1' })).rejects.toThrow('Cannot delete client with 2 active booking(s)');
  });

  it('blocks when unpaid invoices exist', async () => {
    prisma.client.findFirst.mockResolvedValue({ id: 'c1', phone: '+966500000000', notes: '' });
    prisma.invoice.count.mockResolvedValue(1);
    await expect(handler.execute({ clientId: 'c1' })).rejects.toThrow('Cannot delete client with 1 unpaid invoice(s)');
  });

  it('blocks when active group enrollments exist', async () => {
    prisma.client.findFirst.mockResolvedValue({ id: 'c1', phone: '+966500000000', notes: '' });
    prisma.groupEnrollment.count.mockResolvedValue(3);
    await expect(handler.execute({ clientId: 'c1' })).rejects.toThrow('Cannot delete client with 3 active group enrollment(s)');
  });

  it('blocks when waitlist entries exist', async () => {
    prisma.client.findFirst.mockResolvedValue({ id: 'c1', phone: '+966500000000', notes: '' });
    prisma.waitlistEntry.count.mockResolvedValue(1);
    await expect(handler.execute({ clientId: 'c1' })).rejects.toThrow('Cannot delete client with 1 waitlist entry(ies)');
  });

  it('blocks when ratings exist', async () => {
    prisma.client.findFirst.mockResolvedValue({ id: 'c1', phone: '+966500000000', notes: '' });
    prisma.rating.count.mockResolvedValue(5);
    await expect(handler.execute({ clientId: 'c1' })).rejects.toThrow('Cannot delete client with 5 rating(s)');
  });

  it('soft deletes client successfully', async () => {
    prisma.client.findFirst.mockResolvedValue({ id: 'c1', phone: '+966500000000', notes: 'existing note' });
  ...
  it('soft deletes client without phone', async () => {
    prisma.client.findFirst.mockResolvedValue({ id: 'c1', phone: null, notes: 'note' });
    await handler.execute({ clientId: 'c1' });
    expect(prisma.client.update).toHaveBeenCalledWith(
      expect.objectContaining({
