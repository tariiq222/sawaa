...
describe('ClientCancelBookingHandler', () => {
  it('cancels a PENDING booking with >24h notice → CANCELLED', async () => {
    ...
    const handler = new ClientCancelBookingHandler(prisma as never, settings as never, buildEventBus() as never, refundHandler as never);
  ...
  it('throws NotFoundException when booking does not exist', async () => {
    ...
    const handler = new ClientCancelBookingHandler(prisma as never, buildSettingsHandler() as never, buildEventBus() as never, refundHandler as never);
