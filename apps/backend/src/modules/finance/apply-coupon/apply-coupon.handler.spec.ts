...
describe('ApplyCouponHandler', () => {
  ...
  it('throws NotFoundException when invoice not found', async () => {
    ...
    await expect(new ApplyCouponHandler(prisma as never, buildRlsTransaction(prisma) as never).execute(cmd)).rejects.toThrow(NotFoundException);
  });
  ...
  it('throws BadRequestException when coupon expired', async () => {
    ...
    await expect(new ApplyCouponHandler(prisma as never, buildRlsTransaction(prisma) as never).execute(cmd)).rejects.toThrow(BadRequestException);
  });
  ...
  it('proceeds without feature-flag check (COUPONS feature-flag removed in single-tenant migration)', async () => {
    ...
    const prisma = buildPrisma();
    const result = await new ApplyCouponHandler(prisma as never, buildRlsTransaction(prisma) as never).execute(cmd);
    expect(result.id).toBe('red-1');
