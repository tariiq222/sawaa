...
describe('ConfirmBookingHandler', () => {
  it('confirms PENDING booking and emits BookingConfirmedEvent', async () => {
    ...
    await new ConfirmBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never, eb as never, buildZoomHandler() as never).execute({
  ...
  it('throws BadRequestException when booking is already CONFIRMED', async () => {
    ...
    await expect(
      new ConfirmBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never, buildEventBus() as never, buildZoomHandler() as never).execute({
        bookingId: 'book-1', changedBy: 'user-42',
      }),
...
describe('ConfirmBookingHandler — status log', () => {
  it('writes a BookingStatusLog entry on confirm', async () => {
    ...
    const handler = new ConfirmBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never, eventBus as never, buildZoomHandler() as never);
