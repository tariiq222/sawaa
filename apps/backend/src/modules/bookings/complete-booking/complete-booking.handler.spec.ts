...
describe('CompleteBookingHandler', () => {
  it('completes CONFIRMED booking', async () => {
    ...
    await new CompleteBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'book-1', changedBy: 'user-42' });
  ...
  it('throws BadRequestException when booking is not CONFIRMED', async () => {
    ...
    await expect(
      new CompleteBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'book-1', changedBy: 'user-42' }),
  ...
  it('throws NotFoundException when not found', async () => {
    ...
    await expect(
      new CompleteBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never).execute({ bookingId: 'bad', changedBy: 'user-42' }),
...
describe('CompleteBookingHandler — status log', () => {
  it('writes a BookingStatusLog entry on complete', async () => {
    ...
    const handler = new CompleteBookingHandler(prisma as never, buildRlsTx(prisma) as never, buildTenant() as never);
