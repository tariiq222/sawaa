// zoho-credentials — Per-tenant key derivation via HKDF + AES-256-GCM.
//
// Security model:
...
const HKDF_SALT = 'deqah-zoho-creds-v1';
const HKDF_KEY_LEN = 32; // 256 bits

/**
 * Per-tenant envelope encryption for Zoho integration credentials.
 *
 * Uses HKDF-SHA256 to derive a unique AES-256-GCM key per tenant from a
 * shared master key. Each provider has its own HKDF_SALT so a leak of one
 * provider's encrypted blobs is useless for the others.
 *
 * The encrypted blob is the JSON-serialised ZohoIntegrationConfig.
 */
@Injectable()
