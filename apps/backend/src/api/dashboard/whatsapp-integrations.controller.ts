...
import { UnlinkWhatsappHandler } from "../../modules/integrations/whatsapp/unlink-whatsapp.handler";
...
export class WhatsappIntegrationsController {
  constructor(
    ...
    private readonly unlinkConfig: UnlinkWhatsappHandler,
  ) {}

  @Get()
  @CheckPermissions({ action: "read", subject: "Integration" })
  @ApiOperation({
    summary: "Get WhatsApp integration status (no secrets returned)",
  })
  get() {
  ...
  @CheckPermissions({ action: "manage", subject: "Integration" })
  @ApiOperation({
    summary: "Verify Evolution API credentials by fetching connection state",
  })
  @ApiOkResponse({
  ...
  @CheckPermissions({ action: "manage", subject: "Integration" })
  @ApiOperation({
    summary: "Erase WhatsApp credentials and disconnect the agent",
  })
  reset() {
  ...
  @ApiOkResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        logoutOk: { type: "boolean" },
      },
    },
  })
  unlink() {
    return this.unlinkConfig.execute();
  }
}
